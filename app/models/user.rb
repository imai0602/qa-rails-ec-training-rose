class User < ApplicationRecord
  ...
  #電話番号(ハイフン無し) #パスワード 共通で使用(半角数字)
  VALID_PHONE_AND_PASSWORD_REGEX = /\A[0-9]+\z/

  validates :password, presence: true, length: {minimum: 6, maximum: 15}, format: { with: VALID_PHONE_AND_PASSWORD_REGEX }
  ...
  validates :phone_number, presence: true, length: { maximum: 15 }, format: { with: VALID_PHONE_AND_PASSWORD_REGEX }
